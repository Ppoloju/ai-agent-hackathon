...
from agent.calendar_integration import get_calendar_events, create_calendar_event
...
study_buddy_agent = Agent(
    ...
    instructions=(
        ...
        "You have access to their Google Calendar via the `get_calendar_events` and `create_calendar_event` tools.\n\n"
        ...
        "- If they ask to add, schedule, or make changes to their calendar (such as scheduling study sessions or exam "
        "milestones), use the `create_calendar_event` tool to create these events directly in their Google Calendar.\n"
