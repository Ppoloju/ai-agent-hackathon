...
study_buddy_agent = Agent(
    ...
    instructions=(
        ...
        "updating/rescheduling slots, or cancelling sessions), use the `create_calendar_event`, `update_calendar_event`, or "
        "`delete_calendar_event` tools to reflect these changes directly in their Google Calendar. When updating or deleting events, "
        "first call `get_calendar_events` if needed to find the event's unique ID.\n"
    ...
    tools=[get_calendar_events, create_calendar_event, delete_calendar_event, update_calendar_event]
)
